...
const longestString = (arr) => {
  return arr.length? arr.reduce( (lowestIndex, str, idx) => str.length > arr[lowestIndex].length ? idx : lowestIndex, 0) : -1;
};
...
const onlyOddChars = (str) => {
  return str.split('').filter( (item, idx) => idx%2).join('');
};
...
const findEvery = (arr, target) => {
  return arr.reduce((status, item) => item.includes(target) ? status : status = false, true);
};
...
const unenrollBrook = (arr) => {
  let newArr = [];
  for (let i in arr) {
    newArr.push(arr[i].filter(item => !item.includes('Brook')));
  }
  return newArr;
};
